...
def submit_request_for_dates(dates, email, base_data_dir, spec_stem, targ_name, clobber=False):
    ...
    ----------
    dates : str 
       Range of dates e.g. 54858-54979 
    email : str
    ...
    myReq.setGlobalPars(name=targ_name, getTargs=True, getCoords=True, centroid=True,
    ...
    # Get SED per ObsID
    myReq.addSpectrum(hasRedshift=False, whichData='user', specStem=spec_stem, timeslice='single', #TODO user?
                      doNotFit=True,
                      useObs=dates,
                      # For *position*
                      incHours=12)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cfg_fn", type=str, default="mkn421_config.cfg", help="Config filename formatted as in the default; see that file for example.")
    args = parser.parse_args()
    cfg_filename = args.cfg_fn

    _, email, base_data_dir, spec_stem, targ_name, dates = utils.load_cfg(cfg_filename) 

    submit_request_for_dates(dates, email, base_data_dir, spec_stem, targ_name)
